...
def clear_stim_add(): 
 ...
 #save the current words in the entries
 entryWords = []
 
 #obviously, there are only words if the lenght of stimList is greater than zero
 for i in range(len(stimList)):
  entryWords.append(stimList[i].get())
 
 #destroy old frame
 stimScrollingArea.destroy()

 stimScrollingArea = add_stim(main, stimList, entryWords)
 
#Function to remove new stimuli 
def clear_stim_remove(): 
 global stimScrollingArea
 if len(stimList) > 0: #must be greater than 0 to be able to remove a stimulus
   ...
   stimScrollingArea = remove_stim(main, stimList, entryWords)
...
enterStimButton = Button(main, text = 'OK', command = lambda: specify_stim(main, stimList, enterStimBox.get()))
...
delButton = Button(main, text = 'Delete Previous', command = lambda: clear_stim_remove(), width = 23)
delButton.pack(in_=buttonsFrame, side = LEFT)

#Add stimulus Button
addStim = Button(main, text = 'Add new stimulus', 
                 command = lambda: clear_stim_add(), width = 23)
